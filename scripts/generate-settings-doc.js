...
import { escapeBackticks, formatDefaultValue, formatWithPrettier, injectBetweenMarkers, normalizeForCompare, } from './utils/autogen.js';
...
async function updateFile(filePath, newContent, checkOnly) {
    ...
    if (checkOnly) {
        console.error('Settings documentation (' +
...
function collectEntries(schema, options = {}) {
    ...
    const visit = (current, pathSegments, topLevel) => {
        for (const [key, definition] of Object.entries(current)) {
            ...
            const hasChildren = definition.type === 'object' &&
