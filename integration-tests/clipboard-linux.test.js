...
describe('Linux Clipboard Integration', () => {
    ...
    beforeEach(() => {
        ...
        dummyImagePath = path.join(os.tmpdir(), `gemini-test-clipboard-${Date.now()}.png`);
    ...
    runIfLinux('should paste image from system clipboard when Ctrl+V is pressed', async () => {
        // 1. Setup rig
        ...
        if (wlCopy.status === 0) {
            ...
            // Try xclip (X11)
            try {
                execSync(`xclip -selection clipboard -t image/png -i "${dummyImagePath}"`, { stdio: 'ignore' });
                clipboardSet = true;
                sessionType = 'x11';
            }
            catch {
        ...
        if (!clipboardSet) {
            console.warn('Skipping test: Could not access system clipboard (wl-copy or xclip required)');
            return;
        }
...
//# sourceMappingURL=clipboard-linux.test.js.map