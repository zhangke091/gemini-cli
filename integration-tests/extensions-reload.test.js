...
describe('extension reloading', () => {
    ...
    itIf((!sandboxEnv || sandboxEnv === 'false') &&
        platform() !== 'win32' &&
        platform() !== 'linux')('installs a local extension, updates it, checks it was reloaded properly', async () => {
        ...
        await rig.pollCommand(async () => {
            await run.sendText('/extensions list');
            await run.type('\r');
        }, () => stripAnsi(run.output).includes('test-extension (v0.0.2) - active (updated)'), 30000);
