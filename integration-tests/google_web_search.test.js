...
describe('web search tool', () => {
    ...
    it('should be able to search the web', async () => {
        ...
        if (!foundToolCall) {
            ...
            const failedSearchCalls = allTools.filter((t) => t.toolRequest.name === WEB_SEARCH_TOOL_NAME && !t.toolRequest.success);
            if (failedSearchCalls.length > 0) {
                console.warn(`${WEB_SEARCH_TOOL_NAME} tool was called but failed, possibly due to network issues`);
                console.warn('Failed calls:', failedSearchCalls.map((t) => t.toolRequest.args));
                return; // Skip the test if network issues
            }
        }
        expect(foundToolCall, `Expected to find a call to ${WEB_SEARCH_TOOL_NAME}`).toBeTruthy();
