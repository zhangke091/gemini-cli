...
describe('run_shell_command', () => {
    ...
    it.skip('should combine multiple --allowed-tools flags', async () => {
        ...
        const { tool, command } = getLineCountCommand();
        const prompt = `use both ${command} and ls to count the number of lines in files in this ` +
        ...
        for (const expected in ['ls', tool]) {
            const foundToolCall = await rig.waitForToolCall('run_shell_command', 15000, (args) => args.toLowerCase().includes(`"command": "${expected}`));
            if (!foundToolCall) {
                printDebugInfo(rig, result, {
                    'Found tool call': foundToolCall,
                });
            }
            expect(foundToolCall, `Expected to find a run_shell_command tool call to "${expected}",` +
    ...
    it('should reject commands not on the allowlist', async () => {
        ...
        const prompt = `I am testing the allowed tools configuration. ` +
        ...
        const foundToolCall = await rig.waitForToolCall('run_shell_command', 15000, (args) => args.toLowerCase().includes(disallowed.tool.toLowerCase()));
        if (!foundToolCall) {
            printDebugInfo(rig, result, {
                'Found tool call': foundToolCall,
        ...
        const failureLog = toolLogs.find((toolLog) => toolLog.toolRequest.args
            .toLowerCase()
    ...
    it('should allow all with "ShellTool" and other specific tools', async () => {
        ...
        expect(toolCall.toolRequest.success).toBe(true);
        // Validate model output - will throw if no output, warn if missing expected content
        validateModelOutput(result, 'test-allow-all', 'Shell command stdin allow all');
    });
    ...
    it('rejects invalid shell expressions', async () => {
        ...
        const foundToolCall = await rig.waitForToolCall('run_shell_command', 15000, (args) => args.toLowerCase().includes(escapedInvalidCommand.toLowerCase()));
        if (!foundToolCall) {
            printDebugInfo(rig, result, {
                'Found tool call': foundToolCall,
