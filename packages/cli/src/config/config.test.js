...
vi.mock('@google/gemini-cli-core', async () => {
  ...
  return {
    ...
    loadServerHierarchicalMemory: vi.fn(
      (cwd, dirs, debug, fileService, extensionLoader, _maxDirs) => {
...
describe('Approval mode tool exclusion logic', () => {
  ...
  it('should throw an error for invalid approval mode values in loadCliConfig', async () => {
    ...
    const settings = createTestMergedSettings();
    await expect(
      loadCliConfig(settings, 'test-session', invalidArgv),
    ).rejects.toThrow(
...
describe('PolicyEngine nonInteractive wiring', () => {
  ...
  it('should set nonInteractive to true when -p flag is used', async () => {
    ...
    expect(config.isInteractive()).toBe(false);
    expect(config.getPolicyEngine().nonInteractive).toBe(true);
  });
  it('should set nonInteractive to false in interactive mode', async () => {
    ...
    expect(config.isInteractive()).toBe(true);
    expect(config.getPolicyEngine().nonInteractive).toBe(false);
  });
});
