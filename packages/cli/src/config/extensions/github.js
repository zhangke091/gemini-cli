...
export function tryParseGithubUrl(source) {
...
export async function fetchReleaseFromGithub(owner, repo, ref, allowPreRelease) {
    if (ref) {
    ...
    const releases = await fetchJson(`https://api.github.com/repos/${owner}/${repo}/releases?per_page=1`);
    if (releases.length === 0) {
        return null;
    }
    return releases[0];
}
export async function checkForExtensionUpdate(extension, extensionManager) {
    ...
    if (installMetadata?.type === 'local') {
        ...
        try {
            ...
            debugLogger.warn(`Failed to check for update for local extension "${extension.name}". Could not load extension from source path: ${installMetadata.source}. Error: ${getErrorMessage(e)}`);
            return ExtensionUpdateState.NOT_UPDATABLE;
        }
        if (!latestConfig) {
            debugLogger.warn(`Failed to check for update for local extension "${extension.name}". Could not load extension from source path: ${installMetadata.source}`);
            return ExtensionUpdateState.NOT_UPDATABLE;
        }
    ...
    try {
        if (installMetadata.type === 'git') {
            ...
            const releaseData = await fetchReleaseFromGithub(owner, repo, installMetadata.ref, installMetadata.allowPreRelease);
            if (!releaseData) {
                return ExtensionUpdateState.ERROR;
            }
        ...
        debugLogger.error(`Failed to check for updates for extension "${installMetadata.source}": ${getErrorMessage(error)}`);
        return ExtensionUpdateState.ERROR;
    }
}
export async function downloadFromGitHubRelease(installMetadata, destination, githubRepoInfo) {
...
export async function downloadFile(url, dest, options, redirectCount = 0) {
    const headers = {
