...
describe('Policy Engine Integration Tests', () => {
    describe('Policy configuration produces valid PolicyEngine config', () => {
        ...
        it('should handle Plan mode correctly', async () => {
            ...
            expect((await engine.check({ name: 'read_file' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'google_web_search' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            expect((await engine.check({ name: 'list_directory' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            // Other tools should be denied via catch all
            expect((await engine.check({ name: 'replace' }, undefined)).decision).toBe(PolicyDecision.DENY);
            expect((await engine.check({ name: 'write_file' }, undefined)).decision).toBe(PolicyDecision.DENY);
        ...
        it('should allow write_file to plans directory in Plan mode', async () => {
            ...
            const validPlanPath = '/home/user/.gemini/tmp/a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2/plans/my-plan.md';
            expect((await engine.check({ name: 'write_file', args: { file_path: validPlanPath } }, undefined)).decision).toBe(PolicyDecision.ALLOW);
            // Valid plan with underscore in filename
            const validPlanPath2 = '/home/user/.gemini/tmp/a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2/plans/feature_auth.md';
            expect((await engine.check({ name: 'write_file', args: { file_path: validPlanPath2 } }, undefined)).decision).toBe(PolicyDecision.ALLOW);
        });
        it('should deny write_file outside plans directory in Plan mode', async () => {
            ...
            expect((await engine.check({ name: 'write_file', args: { file_path: '/project/src/file.ts' } }, undefined)).decision).toBe(PolicyDecision.DENY);
            // Write to plans dir but wrong extension should be denied
            const wrongExtPath = '/home/user/.gemini/tmp/a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2/plans/script.js';
            expect((await engine.check({ name: 'write_file', args: { file_path: wrongExtPath } }, undefined)).decision).toBe(PolicyDecision.DENY);
            // Path traversal attempt should be denied (filename contains /)
            const traversalPath = '/home/user/.gemini/tmp/a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2/plans/../../../etc/passwd.md';
            expect((await engine.check({ name: 'write_file', args: { file_path: traversalPath } }, undefined)).decision).toBe(PolicyDecision.DENY);
            // Invalid hash length should be denied
            const shortHashPath = '/home/user/.gemini/tmp/abc123/plans/plan.md';
            expect((await engine.check({ name: 'write_file', args: { file_path: shortHashPath } }, undefined)).decision).toBe(PolicyDecision.DENY);
        });
        it('should deny write_file to subdirectories in Plan mode', async () => {
            ...
            const subdirPath = '/home/user/.gemini/tmp/a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2/plans/subdir/plan.md';
            expect((await engine.check({ name: 'write_file', args: { file_path: subdirPath } }, undefined)).decision).toBe(PolicyDecision.DENY);
        });
