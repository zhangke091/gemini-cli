...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    ...
    mockKeychainStorage = {
      ...
      setSecret: vi.fn().mockImplementation(async (key, value) => {
  ...
  describe('installExtension', () => {
    ...
    it('should throw an error for invalid JSON in gemini-extension.json', async () => {
      ...
      await expect(
        ...
        new RegExp(
          `^Failed to load extension config from ${configPath.replace(/\\/g, '\\\\')}`,
        ),
      );
    });
    ...
    it('should show users information on their ansi escaped mcp servers when installing', async () => {
      ...
      expect(mockRequestConsent)
        .toHaveBeenCalledWith(`Installing extension "my-local-extension".
