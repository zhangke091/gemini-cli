...
import { ExtensionUninstallEvent, ExtensionDisableEvent, ExtensionEnableEvent, KeychainTokenStorage, loadAgentsFromDirectory, loadSkillsFromDir, } from '@google/gemini-cli-core';
...
describe('extension tests', () => {
    ...
    describe('loadExtensions', () => {
        ...
        it('should not load github extensions if blockGitExtensions is set', async () => {
            ...
            expect(extension).toBeUndefined();
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Extensions from remote sources is disallowed by your current settings.'));
            consoleSpy.mockRestore();
        });
        ...
        it('should not load disallowed extensions if the allowlist is set.', async () => {
            ...
            const extension = extensions.find((e) => e.name === 'my-ext');
            expect(extension).toBeUndefined();
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('This extension is not allowed by the "allowedExtensions" security setting'));
            consoleSpy.mockRestore();
        });
    ...
    describe('installExtension', () => {
        ...
        it('should not install a disallowed extension if the allowlist is set', async () => {
            ...
            await expect(extensionManager.installOrUpdateExtension({
                source: gitUrl,
                type: 'git',
            })).rejects.toThrow(`Installing extension from source "${gitUrl}" is not allowed by the "allowedExtensions" security setting.`);
        });
