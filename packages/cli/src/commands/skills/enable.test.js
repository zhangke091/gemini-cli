...
describe('skills enable command', () => {
    ...
    describe('handleEnable', () => {
        it('should enable a disabled skill in user scope', async () => {
            ...
            expect(emitConsoleLog).toHaveBeenCalledWith('log', 'Skill "skill1" enabled by removing it from the disabled list in user (/user/settings.json) and workspace (/workspace/settings.json) settings.');
        });
        it('should enable a skill across multiple scopes', async () => {
            ...
            await handleEnable({ name: 'skill1' });
            expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'skills.disabled', []);
            expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'skills.disabled', []);
            expect(emitConsoleLog).toHaveBeenCalledWith('log', 'Skill "skill1" enabled by removing it from the disabled list in workspace (/workspace/settings.json) and user (/user/settings.json) settings.');
        });
        it('should log a message if the skill is already enabled', async () => {
            ...
            expect(emitConsoleLog).toHaveBeenCalledWith('log', 'Skill "skill1" is already enabled.');
        });
    });
