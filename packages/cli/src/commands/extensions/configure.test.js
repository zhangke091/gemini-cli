...
import { updateSetting, promptForSetting, getScopedEnvContents, } from '../../config/extensions/extensionSettings.js';
import prompts from 'prompts';
import * as fs from 'node:fs';
const { mockExtensionManager, mockGetExtensionAndManager, mockGetExtensionManager, mockLoadSettings, } = vi.hoisted(() => {
...
describe('extensions configure command', () => {
    ...
    beforeEach(() => {
        ...
        ExtensionManager.mockImplementation(() => mockExtensionManager);
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    const setupExtension = (name, settings = [], id = 'test-id', path = '/test/path') => {
    ...
    describe('Specific setting configuration', () => {
        it('should configure a specific setting', async () => {
            ...
            expect(updateSetting).toHaveBeenCalledWith(expect.objectContaining({ name: 'test-ext' }), 'test-id', 'TEST_VAR', promptForSetting, 'user', tempWorkspaceDir);
        });
    ...
    describe('Extension configuration (all settings)', () => {
        it('should configure all settings for an extension', async () => {
            ...
            getScopedEnvContents.mockResolvedValue({});
            updateSetting.mockResolvedValue(undefined);
            await runCommand('config test-ext');
            expect(debugLogger.log).toHaveBeenCalledWith('Configuring settings for "test-ext"...');
            expect(updateSetting).toHaveBeenCalledWith(expect.objectContaining({ name: 'test-ext' }), 'test-id', 'VAR_1', promptForSetting, 'user', tempWorkspaceDir);
        });
        it('should verify overwrite if setting is already set', async () => {
            const settings = [{ name: 'Setting 1', envVar: 'VAR_1' }];
            setupExtension('test-ext', settings);
            getScopedEnvContents.mockImplementation(async (_config, _id, scope) => {
                if (scope === 'user')
                    return { VAR_1: 'existing' };
                return {};
            });
        ...
        it('should note if setting is configured in workspace', async () => {
            ...
            getScopedEnvContents.mockImplementation(async (_config, _id, scope) => {
                if (scope === 'workspace')
                    return { VAR_1: 'workspace_value' };
                return {};
            });
            updateSetting.mockResolvedValue(undefined);
            await runCommand('config test-ext');
            expect(debugLogger.log).toHaveBeenCalledWith(expect.stringContaining('is already configured in the workspace scope'));
        });
        it('should skip update if user denies overwrite', async () => {
            const settings = [{ name: 'Setting 1', envVar: 'VAR_1' }];
            setupExtension('test-ext', settings);
            getScopedEnvContents.mockResolvedValue({ VAR_1: 'existing' });
