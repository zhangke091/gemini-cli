...
describe('runNonInteractive', () => {
  ...
  it('should handle cancellation (Ctrl+C)', async () => {
    ...
    const events = [{ type: GeminiEventType.Content, value: 'Thinking...' }];
    // Create a stream that responds to abortion
    mockGeminiClient.sendMessageStream.mockImplementation((_messages, signal) =>
      (async function* () {
        yield events[0];
  ...
  it('should handle EPIPE error gracefully', async () => {
    ...
    const stdoutErrorCallback = process.stdout.on.mock.calls.find(
      (call) => call[0] === 'error',
    )?.[1];
  ...
  it('should resume chat when resumedSessionData is provided', async () => {
    ...
    const resumedSessionData = {
      conversation: {
        ...
        messages: [{ role: 'user', parts: [{ text: 'Previous message' }] }], // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  it.each([
    {
      name: 'loop detected',
      events: [{ type: GeminiEventType.LoopDetected }],
      input: 'Loop test',
      promptId: 'prompt-id-loop',
    },
    {
      name: 'max session turns',
      events: [{ type: GeminiEventType.MaxSessionTurns }],
