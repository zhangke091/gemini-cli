...
describe('QueuedMessageDisplay', () => {
    it('renders nothing when message queue is empty', () => {
        const { lastFrame, unmount } = render(_jsx(QueuedMessageDisplay, { messageQueue: [] }));
        expect(lastFrame()).toBe('');
        unmount();
    });
    it('displays single queued message', () => {
        const { lastFrame, unmount } = render(_jsx(QueuedMessageDisplay, { messageQueue: ['First message'] }));
    ...
    it('displays multiple queued messages', () => {
        ...
        const { lastFrame, unmount } = render(_jsx(QueuedMessageDisplay, { messageQueue: messageQueue }));
        const output = lastFrame();
        expect(output).toContain('Queued (press ↑ to edit):');
...
//# sourceMappingURL=QueuedMessageDisplay.test.js.map