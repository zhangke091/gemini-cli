...
vi.mock('../TerminalOutput.js', () => ({
  TerminalOutput: function MockTerminalOutput({ cursor }) {
    return _jsxs(Text, {
      children: ['MockCursor:(', cursor?.x, ',', cursor?.y, ')'],
    });
  },
}));
...
vi.mock('../GeminiRespondingSpinner.js', () => ({
  GeminiRespondingSpinner: ({ nonRespondingDisplay }) => {
    ...
    return nonRespondingDisplay
      ? _jsx(Text, { children: nonRespondingDisplay })
      : null;
  },
}));
...
describe('<ToolMessage />', () => {
  ...
  describe('JSON rendering', () => {
    ...
    it('renders pretty JSON in ink frame', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, resultDisplay: '{"a":1,"b":2}' }),
        StreamingState.Idle,
      );
  ...
  describe('ToolStatusIndicator rendering', () => {
    it('shows ✓ for Success status', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Success }),
        StreamingState.Idle,
      );
    ...
    it('shows o for Pending status', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Pending }),
        StreamingState.Idle,
      );
      expect(lastFrame()).toMatchSnapshot();
    });
    it('shows ? for Confirming status', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Confirming }),
        StreamingState.Idle,
      );
      expect(lastFrame()).toMatchSnapshot();
    });
    it('shows - for Canceled status', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Canceled }),
        StreamingState.Idle,
      );
      expect(lastFrame()).toMatchSnapshot();
    });
    it('shows x for Error status', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Error }),
        StreamingState.Idle,
      );
      expect(lastFrame()).toMatchSnapshot();
    });
    it('shows paused spinner for Executing status when streamingState is Idle', () => {
      const { lastFrame } = renderWithContext(
        _jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Executing }),
        StreamingState.Idle,
      );
      expect(lastFrame()).toMatchSnapshot();
    });
  ...
  it('renders DiffRenderer for diff results', () => {
    ...
    const { lastFrame } = renderWithContext(
      _jsx(ToolMessage, { ...baseProps, resultDisplay: diffResult }),
      StreamingState.Idle,
    );
  ...
  it('renders emphasis correctly', () => {
    const { lastFrame: highEmphasisFrame } = renderWithContext(
      _jsx(ToolMessage, { ...baseProps, emphasis: 'high' }),
      StreamingState.Idle,
    );
    ...
    const { lastFrame: lowEmphasisFrame } = renderWithContext(
      _jsx(ToolMessage, { ...baseProps, emphasis: 'low' }),
      StreamingState.Idle,
    );
  ...
  it('renders AnsiOutputText for AnsiOutput results', () => {
    ...
    const { lastFrame } = renderWithContext(
      _jsx(ToolMessage, { ...baseProps, resultDisplay: ansiResult }),
      StreamingState.Idle,
    );
    expect(lastFrame()).toMatchSnapshot();
  });
