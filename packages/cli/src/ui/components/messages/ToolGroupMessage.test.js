...
describe('<ToolGroupMessage />', () => {
    ...
    describe('Confirmation Handling', () => {
        ...
        it('renders confirmation with permanent approval disabled', () => {
            ...
            const { lastFrame, unmount } = renderWithProviders(_jsx(ToolGroupMessage, { ...baseProps, toolCalls: toolCalls }), { config: mockConfig });
    ...
    describe('Ask User Filtering', () => {
        ...
        it.each([ToolCallStatus.Success, ToolCallStatus.Error])('does NOT filter out ask_user when status is %s', (status) => {
            const toolCalls = [
                createToolCall({
                    callId: `ask-user-${status}`,
                    name: ASK_USER_DISPLAY_NAME,
