...
const renderWithMockedStats = (metrics, width) => {
    ...
    return render(_jsx(ModelStatsDisplay, {}), width);
};
describe('<ModelStatsDisplay />', () => {
    ...
    it('should render user identity information when provided', () => {
        ...
        const { lastFrame } = render(_jsx(ModelStatsDisplay, { selectedAuthType: "oauth", userEmail: "test@example.com", tier: "Pro" }));
        useSessionStatsMock.mockReturnValue({
            stats: {
                sessionId: 'test-session',
                sessionStartTime: new Date(),
