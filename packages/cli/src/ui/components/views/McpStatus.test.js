...
describe('McpStatus', () => {
  ...
  it('renders correctly with authenticated OAuth status', () => {
    const { lastFrame, unmount } = render(
      _jsx(McpStatus, {
        ...baseProps,
        authStatus: { 'server-1': 'authenticated' },
      }),
    );
    expect(lastFrame()).toMatchSnapshot();
    unmount();
  ...
  it('renders correctly with expired OAuth status', () => {
    const { lastFrame, unmount } = render(
      _jsx(McpStatus, { ...baseProps, authStatus: { 'server-1': 'expired' } }),
    );
    expect(lastFrame()).toMatchSnapshot();
    unmount();
  });
  ...
  it('renders correctly when discovery is in progress', () => {
    const { lastFrame, unmount } = render(
      _jsx(McpStatus, { ...baseProps, discoveryInProgress: true }),
    );
    expect(lastFrame()).toMatchSnapshot();
    unmount();
  });
  it('renders correctly with schema enabled', () => {
    const { lastFrame, unmount } = render(
      _jsx(McpStatus, { ...baseProps, showSchema: true }),
    );
    expect(lastFrame()).toMatchSnapshot();
    unmount();
  });
  ...
  it('renders correctly with a connecting server', () => {
    const { lastFrame, unmount } = render(
      _jsx(McpStatus, { ...baseProps, connectingServers: ['server-1'] }),
    );
    expect(lastFrame()).toMatchSnapshot();
    unmount();
  });
  it('truncates resources when exceeding limit', () => {
    ...
    const { lastFrame, unmount } = render(
      _jsx(McpStatus, { ...baseProps, resources: manyResources }),
    );
