...
describe('<ChatList />', () => {
    it('renders correctly with a list of chats', () => {
        const { lastFrame, unmount } = render(_jsx(ChatList, { chats: mockChats }));
        expect(lastFrame()).toMatchSnapshot();
        unmount();
    });
    it('renders correctly with no chats', () => {
        const { lastFrame, unmount } = render(_jsx(ChatList, { chats: [] }));
    ...
    it('handles invalid date formats gracefully', () => {
        ...
        const { lastFrame, unmount } = render(_jsx(ChatList, { chats: mockChatsWithInvalidDate }));
