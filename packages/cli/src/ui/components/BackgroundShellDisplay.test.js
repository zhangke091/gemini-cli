...
vi.mock('./AnsiOutput.js', () => ({
  AnsiOutputText: ({ data }) => {
    if (typeof data === 'string') return _jsx(_Fragment, { children: data });
...
vi.mock('./shared/ScrollableList.js', () => ({
  ...
  ScrollableList: vi.fn(({ data, renderItem }) =>
    _jsx(Box, {
      flexDirection: 'column',
      children: data.map((item, index) =>
        _jsx(Box, { children: renderItem({ item, index }) }, index),
      ),
    }),
  ),
}));
...
describe('<BackgroundShellDisplay />', () => {
  ...
  it('renders the output of the active shell', async () => {
    const { lastFrame } = render(
      _jsx(ScrollProvider, {
        children: _jsx(BackgroundShellDisplay, {
          shells: mockShells,
