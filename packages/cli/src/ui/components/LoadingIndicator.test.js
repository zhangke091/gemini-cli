...
const renderWithContext = (ui, streamingStateValue, width = 120) => {
  ...
  return render(
    _jsx(StreamingContext.Provider, { value: contextValue, children: ui }),
    width,
  );
};
describe('<LoadingIndicator />', () => {
  ...
  it('should not render when streamingState is Idle', () => {
    const { lastFrame } = renderWithContext(
      _jsx(LoadingIndicator, { ...defaultProps }),
      StreamingState.Idle,
    );
  ...
  it('should transition correctly between states using rerender', () => {
    ...
    rerender(
      _jsx(StreamingContext.Provider, {
        value: StreamingState.Responding,
        children: _jsx(LoadingIndicator, {
    ...
    rerender(
      _jsx(StreamingContext.Provider, {
        value: StreamingState.Idle,
        children: _jsx(LoadingIndicator, { ...defaultProps }),
      }),
    );
    expect(lastFrame()).toBe('');
    unmount();
  ...
  describe('responsive layout', () => {
    it('should render on a single line on a wide terminal', () => {
      const { lastFrame, unmount } = renderWithContext(
        _jsx(LoadingIndicator, {
          ...defaultProps,
          rightContent: _jsx(Text, { children: 'Right' }),
        }),
        StreamingState.Responding,
        120,
      );
