...
vi.mock('./SessionBrowser.js', async (importOriginal) => {
    ...
    const TestSessionBrowser = (props) => {
        const state = original.useSessionBrowserState(props.testSessions || [], false, // Not loading
        ...
        original.useSessionBrowserInput(state, moveSelection, cycleSortOrder, props.onResumeSession, props.onDeleteSession ??
            (async () => {
...
const TestSessionBrowser = SessionBrowser;
...
describe('SessionBrowser component', () => {
    ...
    it('shows empty state when no sessions exist', () => {
        ...
        const onExit = vi.fn();
        const { lastFrame } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [] }));
    ...
    it('renders a list of sessions and marks current session as disabled', () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { lastFrame } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [session1, session2] }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('enters search mode, filters sessions, and renders match snippets', async () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { lastFrame } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [searchSession, otherSession] }));
    ...
    it('does not allow resuming or deleting the current session', () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testSessions: [currentSession, otherSession] }));
    ...
    it('shows an error state when loading sessions fails', () => {
        ...
        const onDeleteSession = vi.fn().mockResolvedValue(undefined);
        const onExit = vi.fn();
        const { lastFrame } = render(_jsx(TestSessionBrowser, { config: config, onResumeSession: onResumeSession, onDeleteSession: onDeleteSession, onExit: onExit, testError: "storage failure" }));
        expect(lastFrame()).toMatchSnapshot();
    });
});
//# sourceMappingURL=SessionBrowser.test.js.map