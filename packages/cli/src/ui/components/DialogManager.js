...
export const DialogManager = ({ addItem, terminalWidth }) => {
  ...
  if (uiState.showIdeRestartPrompt) {
    return _jsx(IdeTrustChangeDialog, {
      reason: uiState.ideTrustRestartReason,
    });
  }
  if (uiState.newAgents) {
    return _jsx(NewAgentsNotification, {
      agents: uiState.newAgents,
      onSelect: uiActions.handleNewAgentsSelect,
    });
  }
  if (uiState.proQuotaRequest) {
    return _jsx(ProQuotaDialog, {
  ...
  if (uiState.shouldShowIdePrompt) {
    return _jsx(IdeIntegrationNudge, {
      ide: uiState.currentIDE,
      onComplete: uiActions.handleIdePromptComplete,
    });
  }
  if (uiState.isFolderTrustDialogOpen) {
    return _jsx(FolderTrustDialog, {
      onSelect: uiActions.handleFolderTrustSelect,
      isRestarting: uiState.isRestarting,
    });
  }
  if (uiState.loopDetectionConfirmationRequest) {
    return _jsx(LoopDetectionConfirmation, {
  ...
  if (
    ...
    return _jsx(Box, {
      flexDirection: 'column',
      children: _jsx(AgentConfigDialog, {
