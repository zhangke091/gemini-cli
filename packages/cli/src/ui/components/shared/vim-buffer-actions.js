 ...
 */
import { getLineRangeOffsets, getPositionFromOffsets, replaceRangeInternal, pushUndo, detachExpandedPaste, isWordCharStrict, isWordCharWithCombining, isCombiningMark, findNextWordAcrossLines, findPrevWordAcrossLines, findWordEndInLine, } from './text-buffer.js';
                ...
                case 'j': {
                    ...
                    if (linesToChange > 0) {
                        if (totalLines === 1) {
                            ...
                            return replaceRangeInternal(detachExpandedPaste(pushUndo(state)), 0, 0, 0, cpLen(currentLine), '');
                        }
                        else {
                            const nextState = detachExpandedPaste(pushUndo(state));
                            const { startOffset, endOffset } = getLineRangeOffsets(cursorRow, linesToChange, nextState.lines);
