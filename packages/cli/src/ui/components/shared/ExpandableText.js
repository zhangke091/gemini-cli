...
const _ExpandableText = ({ label, matchedIndex, userInput = '', textColor = theme.text.primary, isExpanded = false, maxWidth = MAX_WIDTH, maxLines, }) => {
    const hasMatch = matchedIndex !== undefined &&
        matchedIndex >= 0 &&
    ...
    if (!hasMatch) {
        ...
        return (_jsx(Text, { wrap: "wrap", color: textColor, children: display }));
    }
    ...
    return (_jsxs(Text, { color: textColor, wrap: "wrap", children: [before, match
                ? match.split(/(\s+)/).map((part, index) => (_jsx(Text, { color: theme.background.primary, backgroundColor: theme.text.primary, children: part }, `match-${index}`)))
                : null, after] }));
};
