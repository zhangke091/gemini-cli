...
describe('TabHeader', () => {
  describe('rendering', () => {
    ...
    it('renders all tab headers', () => {
      const { lastFrame } = renderWithProviders(
        _jsx(TabHeader, { tabs: MOCK_TABS, currentIndex: 0 }),
      );
  ...
  describe('status icons', () => {
    ...
    it('shows special icon for special tabs', () => {
      const tabsWithSpecial = [
        { key: '0', header: 'Tab 1' },
        { key: '1', header: 'Review', isSpecial: true },
      ];
      const { lastFrame } = renderWithProviders(
        _jsx(TabHeader, { tabs: tabsWithSpecial, currentIndex: 0 }),
      );
      const frame = lastFrame();
    ...
    it('uses tab statusIcon when provided', () => {
      ...
      const { lastFrame } = renderWithProviders(
        _jsx(TabHeader, { tabs: tabsWithCustomIcon, currentIndex: 0 }),
      );
      const frame = lastFrame();
      expect(frame).toContain('★');
      expect(frame).toMatchSnapshot();
...
//# sourceMappingURL=TabHeader.test.js.map
