...
describe('<VirtualizedList />', () => {
  ...
  describe('with 10px height and 100 items', () => {
    ...
    const itemHeight = 1;
    const renderItem1px = ({ item }) =>
      _jsx(Box, {
        height: itemHeight,
        children: _jsx(Text, { children: item }),
      });
    it.each([
      {
        name: 'top',
        ...
        const { lastFrame } = render(
          _jsx(Box, {
            ...
            children: _jsx(VirtualizedList, {
              data: veryLongData,
              renderItem: ({ item }) =>
                _jsx(ItemWithEffect, { item: item }, item),
              keyExtractor: keyExtractor,
          ...
          _jsx(Box, {
            ...
            children: _jsx(VirtualizedList, {
              ...
              renderItem: ({ item, index }) =>
                _jsx(ItemWithContext, { item: item, index: index }),
              keyExtractor: (item) => item.id,
              estimatedItemHeight: () => 1,
            }),
          }),
          _jsx(TestControl, { setFirstItemHeight: setFirstItemHeight }),
        ],
      });
    };
    let setHeightFn = () => {};
