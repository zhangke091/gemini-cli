...
describe('ExpandableText', () => {
    ...
    it('renders plain label when no match (short label)', () => {
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: "simple command", userInput: "", matchedIndex: undefined, textColor: color, isExpanded: false }));
        expect(lastFrame()).toMatchSnapshot();
        unmount();
    });
    it('truncates long label when collapsed and no match', () => {
        const long = 'x'.repeat(MAX_WIDTH + 25);
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: long, userInput: "", textColor: color, isExpanded: false }));
        const out = lastFrame();
        const f = flat(out);
    ...
    it('shows full long label when expanded and no match', () => {
        ...
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: long, userInput: "", textColor: color, isExpanded: true }));
        const out = lastFrame();
        const f = flat(out);
    ...
    it('highlights matched substring when expanded (text only visible)', () => {
        ...
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: label, userInput: userInput, matchedIndex: matchedIndex, textColor: color, isExpanded: true }), 100);
        expect(lastFrame()).toMatchSnapshot();
        unmount();
    });
    it('creates centered window around match when collapsed', () => {
        ...
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: label, userInput: core, matchedIndex: matchedIndex, textColor: color, isExpanded: false }), 100);
        const out = lastFrame();
        const f = flat(out);
    ...
    it('truncates match itself when match is very long', () => {
        ...
        const label = prefix + core + suffix;
        const matchedIndex = prefix.length;
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: label, userInput: core, matchedIndex: matchedIndex, textColor: color, isExpanded: false }));
        const out = lastFrame();
        const f = flat(out);
    ...
    it('respects custom maxWidth', () => {
        ...
        const { lastFrame, unmount } = render(_jsx(ExpandableText, { label: long, userInput: "", textColor: color, isExpanded: false, maxWidth: customWidth }));
        const out = lastFrame();
        const f = flat(out);
        expect(f.endsWith('...')).toBe(true);
...
//# sourceMappingURL=ExpandableText.test.js.map