...
vi.mock('./text-buffer.js', () => {
    const mockTextBuffer = {
        ...
        handleInput: vi.fn((key) => {
            ...
            if (key.sequence) {
                ...
                mockTextBuffer.visualCursor[1] = Math.min(mockTextBuffer.text.length, mockTextBuffer.visualCursor[1] + 1);
            }
        }),
    ...
    return {
        ...
        TextBuffer: vi.fn(() => mockTextBuffer),
    };
});
...
describe('TextInput', () => {
    ...
    it('renders with an initial value', () => {
        ...
        const { lastFrame } = render(_jsx(TextInput, { buffer: buffer, onSubmit: onSubmit, onCancel: onCancel }));
    ...
    it('renders a placeholder', () => {
        ...
        const { lastFrame } = render(_jsx(TextInput, { buffer: buffer, placeholder: "testing", onSubmit: onSubmit, onCancel: onCancel }));
        expect(lastFrame()).toContain('testing');
    });
    it('handles character input', () => {
        render(_jsx(TextInput, { buffer: mockBuffer, onSubmit: onSubmit, onCancel: onCancel }));
    ...
    it('calls onCancel on escape', async () => {
        vi.useFakeTimers();
        render(_jsx(TextInput, { buffer: mockBuffer, onCancel: onCancel, onSubmit: onSubmit }));
        const keypressHandler = mockedUseKeypress.mock.calls[0][0];
        keypressHandler({
            name: 'escape',
            shift: false,
    ...
    it('renders the input value', () => {
        ...
        const { lastFrame } = render(_jsx(TextInput, { buffer: mockBuffer, onSubmit: onSubmit, onCancel: onCancel }));
        expect(lastFrame()).toContain('secret');
    });
    it('does not show cursor when not focused', () => {
        mockBuffer.setText('test');
        const { lastFrame } = render(_jsx(TextInput, { buffer: mockBuffer, focus: false, onSubmit: onSubmit, onCancel: onCancel }));
