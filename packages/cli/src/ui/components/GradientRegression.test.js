...
describe('Gradient Crash Regression Tests', () => {
    it('<Header /> should not crash when theme.ui.gradient is empty', () => {
        const { lastFrame } = renderWithProviders(_jsx(Header, { version: "1.0.0", nightly: false }), {
            width: 120,
        });
        expect(lastFrame()).toBeDefined();
    });
    it('<ModelDialog /> should not crash when theme.ui.gradient is empty', () => {
        const { lastFrame } = renderWithProviders(_jsx(ModelDialog, { onClose: () => { } }), {
            width: 120,
        });
        expect(lastFrame()).toBeDefined();
    });
    it('<Banner /> should not crash when theme.ui.gradient is empty', () => {
        const { lastFrame } = renderWithProviders(_jsx(Banner, { bannerText: "Test Banner", isWarning: false, width: 80 }), {
            width: 120,
        });
        expect(lastFrame()).toBeDefined();
    });
    it('<Footer /> should not crash when theme.ui.gradient has only one color (or empty) and nightly is true', () => {
        const { lastFrame } = renderWithProviders(_jsx(Footer, {}), {
            width: 120,
            uiState: {
    ...
    it('<StatsDisplay /> should not crash when theme.ui.gradient is empty', () => {
        const { lastFrame } = renderWithProviders(_jsx(StatsDisplay, { duration: "1s", title: "My Stats" }), {
            width: 120,
            uiState: {
                sessionStats: mockSessionStats,
            },
...
//# sourceMappingURL=GradientRegression.test.js.map