...
export const ContextSummaryDisplay = ({ geminiMdFileCount, contextFileNames, mcpServers, blockedMcpServers, ideContext, skillCount, backgroundProcessCount = 0, }) => {
    ...
    if (isNarrow) {
        return (_jsx(Box, { flexDirection: "column", paddingX: 1, children: summaryParts.map((part, index) => (_jsxs(Text, { color: theme.text.secondary, children: ["- ", part] }, index))) }));
    }
    return (_jsx(Box, { paddingX: 1, children: _jsx(Text, { color: theme.text.secondary, children: summaryParts.join(' | ') }) }));
};
