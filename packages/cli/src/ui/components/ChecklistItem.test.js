...
describe('<ChecklistItem />', () => {
    ...
    ])('renders %s item correctly', (item) => {
        const { lastFrame } = render(_jsx(ChecklistItem, { item: item }));
    ...
    it('truncates long text when wrap="truncate"', () => {
        ...
        const { lastFrame } = render(_jsx(Box, { width: 30, children: _jsx(ChecklistItem, { item: item, wrap: "truncate" }) }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('wraps long text by default', () => {
        ...
        const { lastFrame } = render(_jsx(Box, { width: 30, children: _jsx(ChecklistItem, { item: item }) }));
        expect(lastFrame()).toMatchSnapshot();
    });
});
