...
describe('IdeIntegrationNudge', () => {
    ...
    it('renders correctly with default options', async () => {
        const { lastFrame } = render(_jsx(KeypressProvider, { children: _jsx(IdeIntegrationNudge, { ...defaultProps }) }));
        await act(async () => {
            await delay(100);
        });
    ...
    it('handles "Yes" selection', async () => {
        ...
        const { stdin } = render(_jsx(KeypressProvider, { children: _jsx(IdeIntegrationNudge, { ...defaultProps, onComplete: onComplete }) }));
        await act(async () => {
            await delay(100);
        });
    ...
    it('displays correct text and handles selection when extension is pre-installed', async () => {
        ...
        const onComplete = vi.fn();
        const { lastFrame, stdin } = render(_jsx(KeypressProvider, { children: _jsx(IdeIntegrationNudge, { ...defaultProps, onComplete: onComplete }) }));
        await act(async () => {
            await delay(100);
        });
        const frame = lastFrame();
...
//# sourceMappingURL=IdeIntegrationNudge.test.js.map