 ...
 */
async function rewindConversation(context, client, recordingService, messageId, newText) {
    try {
        ...
        const startId = 1;
        const historyWithIds = uiHistory.map((item, idx) => ({
            ...item,
            id: startId + idx,
        }));
...
export const rewindCommand = {
    ...
    action: (context) => {
        ...
        return {
            type: 'custom_dialog',
            component: (_jsx(RewindViewer, { conversation: conversation, onExit: () => {
                    context.ui.removeComponent();
