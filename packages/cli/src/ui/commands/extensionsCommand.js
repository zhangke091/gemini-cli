    ...
    for (const name of names) {
        ...
        if (extension?.mcpServers) {
            ...
            if (mcpClientManager && enabledServers.length > 0) {
                const restartPromises = enabledServers.map((serverName) => mcpClientManager.restartServer(serverName).catch((error) => {
                    context.ui.addItem({
                        type: MessageType.WARNING,
...
async function linkAction(context, args) {
    const extensionLoader = context.services.config?.getExtensionLoader();
    if (!(extensionLoader instanceof ExtensionManager)) {
        debugLogger.error(`Cannot ${context.invocation?.name} extensions in this environment`);
        return;
...
async function uninstallAction(context, args) {
    const extensionLoader = context.services.config?.getExtensionLoader();
    if (!(extensionLoader instanceof ExtensionManager)) {
        debugLogger.error(`Cannot ${context.invocation?.name} extensions in this environment`);
        return;
