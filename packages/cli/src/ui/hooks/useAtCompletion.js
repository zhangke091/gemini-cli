...
function buildResourceCandidates(config) {
...
function buildAgentCandidates(config) {
...
async function searchResourceCandidates(pattern, candidates) {
    ...
    const fzf = new AsyncFzf(candidates, {
        selector: (candidate) => candidate.searchKey,
    });
...
async function searchAgentCandidates(pattern, candidates) {
    if (candidates.length === 0) {
        return [];
    }
    const normalizedPattern = pattern.toLowerCase();
...
export function useAtCompletion(props) {
    ...
    useEffect(() => {
        ...
        const search = async () => {
            ...
            const timeoutMs = config?.getFileFilteringOptions()?.searchTimeout ??
            ...
            try {
                ...
                const resourceSuggestions = (await searchResourceCandidates(state.pattern ?? '', resourceCandidates)).map((suggestion) => ({
                    ...suggestion,
