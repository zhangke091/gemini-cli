...
describe('useTurnActivityMonitor', () => {
    beforeEach(() => {
        ...
        vi.mocked(hasRedirection).mockImplementation((query) => query.includes('>') || query.includes('>>'));
    });
    afterEach(() => {
        vi.clearAllMocks();
        vi.useRealTimers();
    ...
    it('should detect redirection from tool calls', () => {
        ...
        vi.mocked(hasRedirection).mockImplementation((q) => q.includes('>'));
        const { result, rerender } = renderHook(({ state, pendingToolCalls }) => useTurnActivityMonitor(state, null, pendingToolCalls), {
            initialProps: {
                state: StreamingState.Responding,
                pendingToolCalls: [],
            },
...
//# sourceMappingURL=useTurnActivityMonitor.test.js.map