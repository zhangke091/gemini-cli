...
describe('handleAtCommand with Agents', () => {
  ...
  it('should detect agent reference and add nudge message', async () => {
    ...
    if (
      ...
      expect(firstPart.text).toContain('Please help me @CodebaseInvestigator');
    } else {
  ...
  it('should handle multiple agents', async () => {
    // Mock another agent
    const mockAgentRegistry = mockConfig.getAgentRegistry();
    mockAgentRegistry.getDefinition = vi.fn((name) => {
      if (name === 'CodebaseInvestigator' || name === 'AnotherAgent') {
        return { name, description: 'desc', kind: 'local' };
      }
      return undefined;
    });
    ...
    expect(nudgePart).toBeDefined();
    if (nudgePart && typeof nudgePart === 'object' && 'text' in nudgePart) {
      expect(nudgePart.text).toContain('CodebaseInvestigator, AnotherAgent');
    }
  });
...
//# sourceMappingURL=atCommandProcessor_agents.test.js.map
