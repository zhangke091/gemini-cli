...
import { ApprovalMode, AuthType, GeminiEventType as ServerGeminiEventType, ToolErrorType, ToolConfirmationOutcome, tokenLimit, debugLogger, coreEvents, CoreEvent, MCPDiscoveryState, } from '@google/gemini-cli-core';
...
describe('useGeminiStream', () => {
    ...
    it('should submit tool responses when all tool calls are completed and ready', async () => {
        ...
        renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
        ...
        expect(mockSendMessageStream).toHaveBeenCalledWith(expectedMergedResponse, expect.any(AbortSignal), 'prompt-id-2', undefined, false, expectedMergedResponse);
    });
    it('should handle all tool calls being cancelled', async () => {
        ...
        renderHookWithProviders(() => useGeminiStream(client, [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
    ...
    it('should not flicker streaming state to Idle between tool completion and submission', async () => {
        ...
        const { result, rerender } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
        ...
        await waitFor(() => {
            expect(mockSendMessageStream).toHaveBeenCalledWith(toolCallResponseParts, expect.any(AbortSignal), 'prompt-id-4', undefined, false, toolCallResponseParts);
        });
    ...
    describe('User Cancellation', () => {
        ...
        it('should call onCancelSubmit handler when escape is pressed', async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHookWithProviders(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, cancelSubmitSpy, () => { }, 80, 24));
            // Start a query
            await act(async () => {
                // eslint-disable-next-line @typescript-eslint/no-floating-promises
                result.current.submitQuery('test query');
        ...
        it('should call setShellInputFocused(false) when escape is pressed', async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHookWithProviders(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, vi.fn(), setShellInputFocusedSpy, // Pass the spy here
            80, 24));
            // Start a query
            await act(async () => {
                // eslint-disable-next-line @typescript-eslint/no-floating-promises
    ...
    describe('Slash Command Handling', () => {
        ...
        it('should call Gemini with prompt content when slash command returns a `submit_prompt` action', async () => {
            ...
            await waitFor(() => {
                ...
                expect(localMockSendMessageStream).toHaveBeenCalledWith('This is the actual prompt from the command file.', expect.any(AbortSignal), expect.any(String), undefined, false, '/my-custom-command');
                expect(mockScheduleToolCalls).not.toHaveBeenCalled();
            });
        });
        it('should correctly handle a submit_prompt action with empty content', async () => {
            ...
            await waitFor(() => {
                ...
                expect(localMockSendMessageStream).toHaveBeenCalledWith('', expect.any(AbortSignal), expect.any(String), undefined, false, '/emptycmd');
            });
        });
        it('should not call handleSlashCommand for line comments', async () => {
            ...
            await waitFor(() => {
                ...
                expect(localMockSendMessageStream).toHaveBeenCalledWith('// This is a line comment', expect.any(AbortSignal), expect.any(String), undefined, false, '// This is a line comment');
            });
        });
        it('should not call handleSlashCommand for block comments', async () => {
            ...
            await waitFor(() => {
                expect(mockHandleSlashCommand).not.toHaveBeenCalled();
                expect(localMockSendMessageStream).toHaveBeenCalledWith('/* This is a block comment */', expect.any(AbortSignal), expect.any(String), undefined, false, '/* This is a block comment */');
            });
        });
        it('should not call handleSlashCommand is shell mode is active', async () => {
            const { result } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, () => { }, mockHandleSlashCommand, true, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
            await act(async () => {
    ...
    describe('Memory Refresh on save_memory', () => {
        it('should call performMemoryRefresh when a save_memory tool call completes successfully', async () => {
            ...
            renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, mockPerformMemoryRefresh, false, () => { }, () => { }, () => { }, 80, 24));
    ...
    describe('Error Handling', () => {
        it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
            ...
            const { result } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(testConfig), [], mockAddItem, testConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
            // 2. Action
            await act(async () => {
    ...
    describe('handleFinishedEvent', () => {
        it('should add info message for MAX_TOKENS finish reason', async () => {
            ...
            const { result } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
            // Submit a query
            await act(async () => {
        ...
        it('should call onCancelSubmit when ContextWindowWillOverflow event is received', async () => {
            ...
            const { result } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, onCancelSubmitSpy, () => { }, 80, 24));
            // Submit a query
            await act(async () => {
                await result.current.submitQuery('Test overflow');
            });
    ...
    it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
        ...
        const { result } = renderHookWithProviders(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, vi.fn(), vi.fn(), false, () => 'vscode', vi.fn(), vi.fn(), false, vi.fn(), vi.fn(), vi.fn(), 80, 24));
        const mockStream = (async function* () {
            yield {
                type: ServerGeminiEventType.Content,
    ...
    it('should process @include commands, adding user turn after processing to prevent race conditions', async () => {
        ...
        const { result } = renderHookWithProviders(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, // shellModeActive
    ...
    describe('Thought Reset', () => {
        ...
        it('should memoize pendingHistoryItems', () => {
            ...
            const { result, rerender } = renderHookWithProviders(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, 80, 24));
    ...
    describe('Loop Detection Confirmation', () => {
        ...
        it('should disable loop detection and show message when user selects "disable"', async () => {
            ...
            await waitFor(() => {
                ...
                expect(mockSendMessageStream).toHaveBeenNthCalledWith(2, 'test query', expect.any(AbortSignal), expect.any(String), undefined, false, 'test query');
            });
        });
        ...
        it('should handle multiple loop detection events properly', async () => {
            ...
            await waitFor(() => {
                ...
                expect(mockSendMessageStream).toHaveBeenNthCalledWith(3, 'second query', expect.any(AbortSignal), expect.any(String), undefined, false, 'second query');
            });
        });
