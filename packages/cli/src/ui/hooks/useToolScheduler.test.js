...
describe('useReactToolScheduler in YOLO Mode', () => {
  ...
  const renderSchedulerInYoloMode = () =>
    renderHook(() =>
      useReactToolScheduler(onComplete, mockConfig, () => undefined),
    );
...
describe('useReactToolScheduler', () => {
  ...
  const renderScheduler = (config = mockConfig) =>
    renderHook(() =>
    ...
    async ({ setup, request, expectedErrorContains, expectedError }) => {
      ...
      if (expectedErrorContains) {
        expectedErrorContains.forEach((errorText) => {
          expect(completedToolCalls[0].response.error.message).toContain(
            errorText,
          );
        });
      }
      if (expectedError) {
        expect(completedToolCalls[0].response.error.message).toBe(
  ...
  it('should handle live output updates', async () => {
    ...
    mockToolWithLiveOutput.execute.mockImplementation(
      async (_args, _signal, updateFn) => {
  ...
  it('should queue if scheduling while already running', async () => {
    mockToolRegistry.getTool.mockReturnValue(mockTool);
    const longExecutePromise = new Promise((resolve) =>
      setTimeout(
        () =>
          resolve({
            llmContent: 'done',
