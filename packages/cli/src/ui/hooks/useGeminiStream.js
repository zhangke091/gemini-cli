...
import { GeminiEventType as ServerGeminiEventType, getErrorMessage, isNodeError, MessageSenderType, logUserPrompt, GitService, UnauthorizedError, UserPromptEvent, DEFAULT_GEMINI_FLASH_MODEL, logConversationFinishedEvent, ConversationFinishedEvent, ApprovalMode, parseAndFormatApiError, ToolConfirmationOutcome, promptIdContext, tokenLimit, debugLogger, runInDevTraceSpan, EDIT_TOOL_NAMES, ASK_USER_TOOL_NAME, processRestorableToolCalls, recordToolCallInteractions, ToolErrorType, ValidationRequiredError, coreEvents, CoreEvent, } from '@google/gemini-cli-core';
...
function isShellToolData(data) {
    ...
    const d = data;
    return ((d.pid === undefined || typeof d.pid === 'number') &&
...
export const useGeminiStream = (geminiClient, history, addItem, config, settings, onDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, onCancelSubmit, setShellInputFocused, terminalWidth, terminalHeight, isShellFocused) => {
    ...
    const pendingToolGroupItems = useMemo(() => {
        ...
        const anyVisibleInPending = remainingTools.some((tc) => {
            ...
            const isInProgress = tc.status !== 'success' &&
                tc.status !== 'error' &&
                tc.status !== 'cancelled';
