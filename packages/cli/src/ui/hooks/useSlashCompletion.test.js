...
describe('useSlashCompletion', () => {
    ...
    describe('Top-Level Commands', () => {
        ...
        it('should suggest commands based on partial altNames', async () => {
            ...
            let unmount;
            await act(async () => {
                const hook = renderHook(() => useTestHarnessForSlashCompletion(true, '/usage', slashCommands, mockCommandContext));
                result = hook.result;
                unmount = hook.unmount;
            });
            await waitFor(() => {
        ...
        it.each([['/?'], ['/usage']])('should suggest commands even when altNames is fully typed', async (query) => {
        ...
        it('should show all matching suggestions even when one is a perfect match', async () => {
            ...
            const { result, unmount } = renderHook(() => useTestHarnessForSlashCompletion(true, '/review', slashCommands, mockCommandContext));
            await waitFor(() => {
        ...
        it('should suggest subcommands when a parent command is fully typed without a trailing space', async () => {
            ...
            const { result, unmount } = renderHook(() => useTestHarnessForSlashCompletion(true, '/chat', slashCommands, mockCommandContext));
            await waitFor(() => {
                // Should show subcommands of 'chat'
                expect(result.current.suggestions).toHaveLength(2);
                expect(result.current.suggestions.map((s) => s.label)).toEqual(expect.arrayContaining(['list', 'save']));
