...
export function useReverseSearchCompletion(buffer, history, reverseSearchActive) {
    const { suggestions, activeSuggestionIndex, visibleStartIndex, showSuggestions, isLoadingSuggestions, setSuggestions, setShowSuggestions, setActiveSuggestionIndex, resetCompletionState, navigateUp, navigateDown, setVisibleStartIndex, } = useCompletion();
    ...
    const searchHistory = useCallback((query, items) => {
        const out = [];
    ...
    const matches = useMemo(() => {
        ...
        const canUseCache = prevQueryRef.current &&
