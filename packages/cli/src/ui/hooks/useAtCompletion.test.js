...
import { FileSearchFactory, FileDiscoveryService, } from '@google/gemini-cli-core';
...
describe('useAtCompletion', () => {
    ...
    describe('MCP resource suggestions', () => {
        it('should include MCP resources in the suggestion list using fuzzy matching', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'logs', mockConfig, '/tmp'));
            await waitFor(() => {
                expect(result.current.suggestions.some((suggestion) => suggestion.value === 'server-1:file:///tmp/server-1/logs.txt')).toBe(true);
            });
        });
    });
