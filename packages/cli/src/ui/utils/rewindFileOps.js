...
import { coreEvents, debugLogger, getFileDiffFromResultDisplay, computeAddedAndRemovedLines, } from '@google/gemini-cli-core';
/**
 ...
 */
export function calculateTurnStats(conversation, userMessage) {
 ...
 */
export function calculateRewindImpact(conversation, userMessage) {
    const msgIndex = conversation.messages.indexOf(userMessage);
    if (msgIndex === -1)
        return null;
    let addedLines = 0;
 ...
 */
export async function revertFileChanges(conversation, targetMessageId) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if ('code' in error && error.code === 'ENOENT') {
                                ...
                                debugLogger.debug(`File ${fileName} not found during revert, proceeding as it may be a new file deletion.`);
                            }
                            else {
                        ...
                        // 2. Mismatch: Attempt Smart Revert (Patch)
                            ...
                            if (typeof patchedContent === 'string') {
                                ...
                                // Patch failed
                                coreEvents.emitFeedback('warning', `Smart revert for ${fileName} failed. The file may have been modified in a way that conflicts with the undo operation.`);
                            }
                        }
                        else {
                            ...
                            coreEvents.emitFeedback('warning', `Cannot revert changes for ${fileName} because it was not found on disk. This is expected if a file created by the agent was deleted before rewind`);
                        }
                    }
                    catch (e) {
...
//# sourceMappingURL=rewindFileOps.js.map