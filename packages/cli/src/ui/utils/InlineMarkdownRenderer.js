...
const RenderInlineInternal = ({ text, defaultColor }) => {
  ...
  while ((match = inlineRegex.exec(text)) !== null) {
    if (match.index > lastIndex) {
      nodes.push(
        _jsx(
          Text,
          { color: baseColor, children: text.slice(lastIndex, match.index) },
          `t-${lastIndex}`,
        ),
      );
    }
      ...
      if (
        ...
        renderedNode = _jsx(
          Text,
          {
            bold: true,
            color: baseColor,
            children: fullMatch.slice(BOLD_MARKER_LENGTH, -BOLD_MARKER_LENGTH),
          },
          key,
        );
      } else if (
        ...
        if (codeMatch && codeMatch[2]) {
          renderedNode = _jsx(
            Text,
            { color: theme.text.accent, children: codeMatch[2] },
            key,
          );
        }
      } else if (
        ...
        renderedNode = _jsx(
          Text,
          { color: theme.text.link, children: fullMatch },
          key,
        );
      }
    } catch (e) {
    ...
    nodes.push(
      renderedNode ??
        _jsx(Text, { color: baseColor, children: fullMatch }, key),
    );
  ...
  if (lastIndex < text.length) {
    nodes.push(
      _jsx(
        Text,
        { color: baseColor, children: text.slice(lastIndex) },
        `t-${lastIndex}`,
      ),
    );
  }
