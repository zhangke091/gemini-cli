  ...
  lines.forEach((line, index) => {
    ...
    if (codeFenceMatch) {
      ...
      addContentBlock(
        _jsx(
          Box,
          { children: _jsx(Text, { dimColor: true, children: '---' }) },
          key,
        ),
      );
    } else if (headerMatch) {
      ...
      if (headerNode) addContentBlock(_jsx(Box, { children: headerNode }, key));
    } else if (ulMatch) {
      ...
      if (line.trim().length === 0 && !inCodeBlock) {
        if (!lastLineEmpty) {
          contentBlocks.push(
            _jsx(Box, { height: EMPTY_LINE_HEIGHT }, `spacer-${index}`),
          );
          lastLineEmpty = true;
        }
      } else {
...
const RenderListItemInternal = ({
  ...
  return _jsxs(Box, {
    ...
    children: [
      _jsx(Box, {
        width: prefixWidth,
        children: _jsx(Text, { color: listResponseColor, children: prefix }),
      }),
      _jsx(Box, {
        flexGrow: LIST_ITEM_TEXT_FLEX_GROW,
        children: _jsx(Text, {
          wrap: 'wrap',
          color: listResponseColor,
          children: _jsx(RenderInline, {
            text: itemText,
