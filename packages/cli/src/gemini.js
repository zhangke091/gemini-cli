...
import { sessionId, logUserPrompt, AuthType, getOauthClient, UserPromptEvent, debugLogger, recordSlowRender, coreEvents, CoreEvent, createWorkingStdio, patchStdio, writeToStdout, writeToStderr, disableMouseEvents, enableMouseEvents, enterAlternateScreen, disableLineWrapping, shouldEnterAlternateScreen, startupProfiler, ExitCodes, SessionStartSource, SessionEndReason, getVersion, ValidationCancelledError, ValidationRequiredError, } from '@google/gemini-cli-core';
...
export async function main() {
    ...
    if (!settings.merged.security.auth.useExternal) {
        try {
            if (partialConfig.isInteractive() &&
                ...
                const authType = await validateNonInteractiveAuth(settings.merged.security.auth.selectedType, settings.merged.security.auth.useExternal, partialConfig, settings);
    ...
    // may have side effects.
    {
        ...
        if (config.isInteractive() && config.storage && config.getDebugMode()) {
            const { registerActivityLogger } = await import('./utils/activityLogger.js');
        ...
        if (argv.resume) {
            ...
            try {
                ...
                coreEvents.emitFeedback('error', `Error resuming session: ${error instanceof Error ? error.message : 'Unknown error'}`);
                await runExitCleanup();
                process.exit(ExitCodes.FATAL_INPUT_ERROR);
            }
        }
        ...
        const authType = await validateNonInteractiveAuth(settings.merged.security.auth.selectedType, settings.merged.security.auth.useExternal, config, settings);
        await config.refreshAuth(authType);
        if (config.getDebugMode()) {
