...
import { debugLogger, Storage, TOOL_OUTPUT_DIR, } from '@google/gemini-cli-core';
        ...
        for (const sessionToDelete of sessionsToDelete) {
            try {
                ...
                if (error instanceof Error &&
                    ...
                    debugLogger.warn(`Failed to delete session ${sessionId}: ${errorMessage}`);
                    result.failed++;
                }
            }
        }
 ...
 */
export async function cleanupToolOutputFiles(settings, debugMode = false, projectTempDir) {
    const result = {
        disabled: false,
        scanned: 0,
        deleted: 0,
    ...
    try {
        ...
        const retentionConfig = settings.general.sessionRetention;
        const tempDir = projectTempDir ?? new Storage(process.cwd()).getProjectTempDir();
        ...
        const fileStatsResults = await Promise.all(files.map(async (file) => {
            try {
                ...
                debugLogger.debug(`Failed to stat file ${file.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                return null;
            }
        }));
        const fileStats = fileStatsResults.filter((f) => f !== null);
        ...
        if (retentionConfig.maxAge) {
            try {
                ...
                debugLogger.debug(`Invalid maxAge format, skipping age-based cleanup: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        ...
        for (const fileName of filesToDelete) {
            try {
                ...
                debugLogger.debug(`Failed to delete file ${fileName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                result.failed++;
            }
        }
