...
import { SessionSelector, extractFirstUserMessage, formatRelativeTime, hasUserOrAssistantMessage, SessionError, } from './sessionUtils.js';
...
describe('SessionSelector', () => {
    ...
    it('should resolve session by UUID', async () => {
        ...
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionId1.slice(0, 8)}.json`), JSON.stringify(session1, null, 2));
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T11-00-${sessionId2.slice(0, 8)}.json`), JSON.stringify(session2, null, 2));
    ...
    it('should deduplicate sessions by ID', async () => {
        ...
        // File 1
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionId.slice(0, 8)}.json`), JSON.stringify(sessionOriginal, null, 2));
        // File 2 (Simulate a copy or newer version with same ID)
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T11-00-${sessionId.slice(0, 8)}.json`), JSON.stringify(sessionDuplicate, null, 2));
        const sessionSelector = new SessionSelector(config);
    ...
    it('should not list sessions with only system messages', async () => {
        ...
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionIdWithUser.slice(0, 8)}.json`), JSON.stringify(sessionWithUser, null, 2));
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T11-00-${sessionIdSystemOnly.slice(0, 8)}.json`), JSON.stringify(sessionSystemOnly, null, 2));
        const sessionSelector = new SessionSelector(config);
        const sessions = await sessionSelector.listSessions();
    ...
    it('should list session with gemini message even without user message', async () => {
        ...
        await fs.writeFile(path.join(chatsDir, `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionIdGeminiOnly.slice(0, 8)}.json`), JSON.stringify(sessionGeminiOnly, null, 2));
        const sessionSelector = new SessionSelector(config);
        const sessions = await sessionSelector.listSessions();
        expect(sessions.length).toBe(1);
...
//# sourceMappingURL=sessionUtils.test.js.map