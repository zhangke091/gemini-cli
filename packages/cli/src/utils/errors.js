 ...
 */
import { OutputFormat, JsonFormatter, StreamJsonFormatter, JsonStreamEventType, uiTelemetryService, parseAndFormatApiError, FatalTurnLimitedError, FatalCancellationError, FatalToolExecutionError, isFatalToolError, debugLogger, coreEvents, } from '@google/gemini-cli-core';
...
export function handleError(error, config, customErrorCode) {
    ...
    if (config.getOutputFormat() === OutputFormat.STREAM_JSON) {
        ...
        const errorCode = customErrorCode ?? extractErrorCode(error);
        const formattedError = formatter.formatError(error instanceof Error ? error : new Error(getErrorMessage(error)), errorCode, config.getSessionId());
 ...
 */
export function handleToolError(toolName, toolError, config, errorType, resultDisplay) {
    ...
    if (isFatal) {
        ...
        if (config.getOutputFormat() === OutputFormat.STREAM_JSON) {
            ...
            const formatter = new JsonFormatter();
            const formattedError = formatter.formatError(toolExecutionError, errorType ?? toolExecutionError.exitCode, config.getSessionId());
            coreEvents.emitFeedback('error', formattedError);
        }
        else {
