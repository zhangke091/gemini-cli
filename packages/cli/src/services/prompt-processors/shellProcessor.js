 ...
 */
import { escapeShellArg, getShellConfiguration, ShellExecutionService, flatMapTextParts, PolicyDecision, } from '@google/gemini-cli-core';
    ...
    async processString(prompt, context) {
        ...
        for (const injection of resolvedInjections) {
            ...
            if (injection.resolvedCommand) {
                ...
                const { result } = await ShellExecutionService.execute(injection.resolvedCommand, config.getTargetDir(), () => { }, new AbortController().signal, config.getEnableInteractiveShell(), shellExecutionConfig);
