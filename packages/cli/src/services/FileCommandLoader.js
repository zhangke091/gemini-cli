...
export class FileCommandLoader {
    ...
    async parseAndAdaptFile(filePath, baseDir, extensionName, extensionId) {
        ...
        try {
            ...
            coreEvents.emitFeedback('error', `[FileCommandLoader] Failed to read file ${filePath}:`, error instanceof Error ? error.message : String(error));
            return null;
        }
        let parsed;
        try {
            ...
            coreEvents.emitFeedback('error', `[FileCommandLoader] Failed to parse TOML file ${filePath}:`, error instanceof Error ? error.message : String(error));
            return null;
        }
        const validationResult = TomlCommandDefSchema.safeParse(parsed);
        if (!validationResult.success) {
            coreEvents.emitFeedback('error', `[FileCommandLoader] Skipping invalid command file: ${filePath}. Validation errors:`, validationResult.error.flatten());
            return null;
        }
        ...
        return {
            ...
            action: async (context, _args) => {
                if (!context.invocation) {
                    coreEvents.emitFeedback('error', `[FileCommandLoader] Critical error: Command '${baseCommandName}' was executed without invocation context.`);
                    return {
                        type: 'submit_prompt',
