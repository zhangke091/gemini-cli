...
export class InteractiveRun {
    ...
    async type(text) {
        ...
        for (const char of text) {
            ...
            const found = await poll(() => stripAnsi(this.output).includes(typedSoFar), 5000, // 5s timeout per character (generous for CI)
            10);
            if (!found) {
                throw new Error(`Timed out waiting for typed text to appear in output: "${typedSoFar}".\nStripped output:\n${stripAnsi(this.output)}`);
            }
        }
    }
...
export class TestRig {
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                ...
                if (code === 0) {
                    ...
                    const finalResult = stderr && !isJsonOutput
                        ? `${result}\n\nStdErr:\n${stderr}`
                        : result;
                    resolve(finalResult);
                }
    ...
    async runInteractive(options) {
        ...
        const ptyOptions = {
            ...
            rows: 80,
            cwd: this.testDir,
            env: Object.fromEntries(Object.entries(envVars).filter(([, v]) => v !== undefined)),
        };
