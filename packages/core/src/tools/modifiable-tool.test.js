...
describe('modifyWithEditor', () => {
    ...
    describe('successful modification', () => {
        const assertMode = (mode, expected) => {
    ...
    it('should honor override content values when provided', async () => {
        ...
        expect(mockCreatePatch).toHaveBeenCalledWith(path.basename(mockParams.filePath), overrideCurrent, modifiedContent, 'Current', 'Proposed', expect.any(Object));
    });
    it('should treat null override as explicit empty content', async () => {
        ...
        expect(mockModifyContext.getCurrentContent).not.toHaveBeenCalled();
        expect(mockModifyContext.getProposedContent).not.toHaveBeenCalled();
        expect(mockCreatePatch).toHaveBeenCalledWith(path.basename(mockParams.filePath), '', modifiedContent, 'Current', 'Proposed', expect.any(Object));
    });
    it('should clean up temp files even if editor fails', async () => {
        ...
        await expect(modifyWithEditor(mockParams, mockModifyContext, DEFAULT_GUI_EDITOR, abortSignal)).rejects.toThrow('Editor failed to open');
