...
describe('WriteFileTool', () => {
    ...
    describe('getCorrectedFileContent', () => {
        it('should call ensureCorrectFileContent for a new file', async () => {
            ...
            expect(mockEnsureCorrectFileContent).toHaveBeenCalledWith(proposedContent, mockBaseLlmClientInstance, abortSignal, true);
        ...
        it('should call ensureCorrectEdit for an existing file', async () => {
            ...
            expect(mockEnsureCorrectEdit).toHaveBeenCalledWith(filePath, originalContent, {
                ...
                file_path: filePath,
            }, mockGeminiClientInstance, mockBaseLlmClientInstance, abortSignal, true);
    ...
    describe('specific error types for write failures', () => {
        ...
        ])('should return $errorType error when write fails with $errorCode', async ({ errorCode, errorType, errorMessage, expectedMessagePrefix, mockFsExistsSync, }) => {
            const filePath = path.join(rootDir, `${errorType}_file.txt`);
            const content = 'test content';
            let existsSyncSpy = undefined;
            try {
                if (mockFsExistsSync) {
    ...
    describe('disableLLMCorrection', () => {
        ...
        it('should call ensureCorrectEdit with disableLLMCorrection=true for an existing file when disabled', async () => {
            ...
            const result = await getCorrectedFileContent(mockConfig, filePath, proposedContent, abortSignal);
            expect(mockEnsureCorrectEdit).toHaveBeenCalledWith(filePath, originalContent, expect.anything(), // params object
            mockGeminiClientInstance, mockBaseLlmClientInstance, abortSignal, true);
            expect(mockEnsureCorrectFileContent).not.toHaveBeenCalled();
            expect(result.correctedContent).toBe(proposedContent);
            expect(result.originalContent).toBe(originalContent);
