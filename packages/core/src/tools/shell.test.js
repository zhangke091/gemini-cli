...
describe('ShellTool', () => {
    ...
    beforeEach(() => {
        ...
        bus.subscribe(MessageBusType.UPDATE_POLICY, (msg) => {
    ...
    describe('execute', () => {
        ...
        it('should wrap command on linux and parse pgrep output', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, tempRootDir, expect.any(Function), expect.any(AbortSignal), false, { pager: 'cat', sanitizationConfig: {} });
        ...
        it('should use the provided absolute directory as cwd', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, subdir, expect.any(Function), expect.any(AbortSignal), false, { pager: 'cat', sanitizationConfig: {} });
        });
        it('should use the provided relative directory as cwd', async () => {
            ...
            const tmpFile = path.join(os.tmpdir(), 'shell_pgrep_abcdef.tmp');
            const wrappedCommand = `{ ls; }; __code=$?; pgrep -g 0 >${tmpFile} 2>&1; exit $__code;`;
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, path.join(tempRootDir, 'subdir'), expect.any(Function), expect.any(AbortSignal), false, { pager: 'cat', sanitizationConfig: {} });
        });
        ...
        itWindowsOnly('should not wrap command on windows', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith('dir', tempRootDir, expect.any(Function), expect.any(AbortSignal), false, { pager: 'cat', sanitizationConfig: {} });
        }, 20000);
