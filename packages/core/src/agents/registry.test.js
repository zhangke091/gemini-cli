...
import { DEFAULT_GEMINI_FLASH_LITE_MODEL, DEFAULT_GEMINI_MODEL, DEFAULT_THINKING_MODE, PREVIEW_GEMINI_FLASH_MODEL, PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_MODEL_AUTO, } from '../config/models.js';
...
function makeMockedConfig(params) {
...
class TestableAgentRegistry extends AgentRegistry {
    async testRegisterAgent(definition) {
...
describe('AgentRegistry', () => {
    ...
    describe('initialize', () => {
        ...
        it('should log the count of loaded agents in debug mode', async () => {
            ...
            expect(debugLogSpy).toHaveBeenCalledWith(`[AgentRegistry] Loaded with ${agentCount} agents.`);
        });
        it('should use default model for codebase investigator for non-preview models', async () => {
            ...
            expect(investigatorDef?.modelConfig.generateContentConfig?.thinkingConfig).toStrictEqual({
                includeThoughts: true,
        ...
        it('should use agentCardUrl as hash for acknowledgement of remote agents', async () => {
            ...
            vi.spyOn(mockConfig, 'getAcknowledgedAgentsService').mockReturnValue(ackService);
            ...
            expect(ackService.isAcknowledged).toHaveBeenCalledWith(expect.anything(), 'RemoteAgent', 'https://example.com/card');
            ...
            expect(registry.getDefinition('RemoteAgent')?.metadata?.hash).toBe('https://example.com/card');
        });
    });
    describe('registration logic', () => {
        it('should register runtime overrides when the model is "auto"', async () => {
            ...
            const registerOverrideSpy = vi.spyOn(mockConfig.modelConfigService, 'registerRuntimeModelOverride');
            ...
            expect(registerOverrideSpy).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should register a valid agent definition', async () => {
            ...
            expect(mockConfig.modelConfigService.getResolvedConfig({
                ...
                generateContentConfig: {
                    temperature: MOCK_AGENT_V1.modelConfig.generateContentConfig?.temperature,
        ...
        it('should log remote agent registration in debug mode', async () => {
            ...
            expect(debugLogSpy).toHaveBeenCalledWith(`[AgentRegistry] Registered remote agent 'RemoteAgent' with card: https://example.com/card`);
        });
        ...
        it('should handle bulk registrations correctly', async () => {
            const promises = Array.from({ length: 100 }, (_, i) => registry.testRegisterAgent({
                ...MOCK_AGENT_V1,
                name: `Agent${i}`,
            }));
        ...
        it('should dynamically register an ALLOW policy for local agents', async () => {
            ...
            expect(addRuleSpy).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should not register a policy if a USER policy already exists', async () => {
            ...
            vi.spyOn(policyEngine, 'hasRuleForTool').mockImplementation((toolName, ignoreDynamic) => toolName === 'ExistingUserPolicyAgent' && ignoreDynamic === true);
            const addRuleSpy = vi.spyOn(policyEngine, 'addRule');
            await registry.testRegisterAgent(agent);
        ...
        it('should replace an existing dynamic policy when an agent is overwritten', async () => {
            ...
            expect(addRuleSpy).toHaveBeenLastCalledWith(expect.objectContaining({ decision: PolicyDecision.ALLOW }));
            // 2. Overwrite with remote
            await registry.testRegisterAgent(remoteAgent);
            // Verify old dynamic rule was removed
            expect(removeRuleSpy).toHaveBeenCalledWith('OverwrittenAgent', 'AgentRegistry (Dynamic)');
            // Verify new dynamic rule (remote -> ASK_USER) was added
            expect(addRuleSpy).toHaveBeenLastCalledWith(expect.objectContaining({
    ...
    describe('accessors', () => {
        ...
        it('getAllDiscoveredAgentNames should return all names including disabled ones', async () => {
            ...
            const registryWithDisabled = new TestableAgentRegistry(configWithDisabled);
        ...
        it('getDiscoveredDefinition should return the definition for a disabled agent', async () => {
            ...
            await registryWithDisabled.testRegisterAgent(disabledAgent);
            expect(registryWithDisabled.getDefinition('DisabledAgent')).toBeUndefined();
            const discovered = registryWithDisabled.getDiscoveredDefinition('DisabledAgent');
    ...
    describe('overrides', () => {
        ...
        it('should merge runConfig overrides', async () => {
            ...
            const registry = new TestableAgentRegistry(config);
            await registry.testRegisterAgent(MOCK_AGENT_V1);
            const def = registry.getDefinition('MockAgent');
            expect(def.runConfig.maxTurns).toBe(50);
            expect(def.runConfig.maxTimeMinutes).toBe(MOCK_AGENT_V1.runConfig.maxTimeMinutes);
        });
        ...
        it('should preserve lazy getters when applying overrides', async () => {
            ...
            const registeredDef = registry.getDefinition('GetterAgent');
    ...
    describe('getDirectoryContext', () => {
        it('should return default message when no agents are registered', () => {
            expect(registry.getDirectoryContext()).toContain('No sub-agents are currently available.');
        });
