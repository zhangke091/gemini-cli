...
describe('FakeContentGenerator', () => {
  ...
  it('should throw error when no more responses', async () => {
    ...
    await expect(generator.embedContent({})).rejects.toThrowError(
      'No more mock responses for embedContent',
    );
    await expect(generator.countTokens({})).rejects.toThrowError(
      'No more mock responses for countTokens',
    );
    await expect(generator.generateContentStream({}, 'id')).rejects.toThrow(
      'No more mock responses for generateContentStream',
    );
    await expect(generator.generateContent({}, 'id')).rejects.toThrowError(
      'No more mock responses for generateContent',
    );
  });
  describe('fromFile', () => {
