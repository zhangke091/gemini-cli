...
\`\`\`
      `.trim();
        ...
        it('should not add context if ideMode is enabled but no open files', async () => {
            ...
            // Assert
            expect(ideContextStore.get).toHaveBeenCalled();
            expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'default-routed-model' }, initialRequest, expect.any(AbortSignal), undefined);
        });
      ...
      `.trim();
        ...
        describe('Model Routing', () => {
            ...
            it('should use the model router service to select a model on the first turn', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'routed-model' }, [{ text: 'Hi' }], expect.any(AbortSignal), undefined);
            });
            it('should use the same model for subsequent turns in the same prompt (stickiness)', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'routed-model' }, [{ text: 'Continue' }], expect.any(AbortSignal), undefined);
            });
            it('should reset the sticky model and re-route when the prompt_id changes', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'new-routed-model' }, [{ text: 'A new topic' }], expect.any(AbortSignal), undefined);
            });
            it('should re-route within the same prompt when the configured model changes', async () => {
                ...
                await fromAsync(stream);
                expect(mockRouterService.route).toHaveBeenCalledTimes(1);
                expect(mockTurnRunFn).toHaveBeenNthCalledWith(1, { model: 'original-model' }, [{ text: 'Hi' }], expect.any(AbortSignal), undefined);
                mockRouterService.route.mockResolvedValue({
                ...
                await fromAsync(stream);
                expect(mockRouterService.route).toHaveBeenCalledTimes(2);
                expect(mockTurnRunFn).toHaveBeenNthCalledWith(2, { model: 'fallback-model' }, [{ text: 'Continue' }], expect.any(AbortSignal), undefined);
            });
        });
        ...
        it('should recursively call sendMessageStream with "Please continue." when InvalidStream event is received', async () => {
            ...
            expect(mockTurnRunFn).toHaveBeenNthCalledWith(1, { model: 'default-routed-model' }, initialRequest, expect.any(AbortSignal), undefined);
            // Second call with "Please continue."
            expect(mockTurnRunFn).toHaveBeenNthCalledWith(2, { model: 'default-routed-model' }, [{ text: 'System: Please continue.' }], expect.any(AbortSignal), undefined);
        });
