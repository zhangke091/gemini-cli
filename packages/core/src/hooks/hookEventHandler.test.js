...
describe('HookEventHandler', () => {
    ...
    beforeEach(() => {
        ...
        hookEventHandler = new HookEventHandler(mockConfig, mockHookPlanner, mockHookRunner, mockHookAggregator);
    });
    describe('fireBeforeToolEvent', () => {
        it('should fire BeforeTool event with correct input', async () => {
            ...
            const onHookEnd = vi.mocked(mockHookRunner.executeHooksParallel).mock
                .calls[0][4];
            if (onHookStart)
                onHookStart(mockPlan[0].hookConfig, 0);
            ...
            if (onHookEnd)
                onHookEnd(mockPlan[0].hookConfig, mockResults[0]);
        ...
        it('should fire BeforeTool event with MCP context when provided', async () => {
            ...
            const result = await hookEventHandler.fireBeforeToolEvent('my-mcp-server__read_file', { path: '/etc/passwd' }, mcpContext);
            expect(mockHookRunner.executeHooksParallel).toHaveBeenCalledWith([mockPlan[0].hookConfig], HookEventName.BeforeTool, expect.objectContaining({
                session_id: 'test-session',
                cwd: '/test/project',
                hook_event_name: 'BeforeTool',
    ...
    describe('fireAfterToolEvent', () => {
        ...
        it('should fire AfterTool event with MCP context when provided', async () => {
            ...
            const result = await hookEventHandler.fireAfterToolEvent('my-mcp-server__read_file', toolInput, toolResponse, mcpContext);
            expect(mockHookRunner.executeHooksParallel).toHaveBeenCalledWith([mockPlan[0].hookConfig], HookEventName.AfterTool, expect.objectContaining({
                tool_name: 'my-mcp-server__read_file',
                tool_input: toolInput,
                tool_response: toolResponse,
    ...
    describe('failure suppression', () => {
        it('should suppress duplicate feedback for the same failing hook and request context', async () => {
            ...
            await hookEventHandler.fireAfterModelEvent(llmRequest, llmResponse);
            ...
            await hookEventHandler.fireAfterModelEvent(differentRequest, llmResponse);
