...
describe('HookRegistry', () => {
    ...
    describe('malformed configuration handling', () => {
        ...
        it('should skip known config fields and warn on invalid event names', async () => {
            ...
            vi.mocked(mockConfig.getHooks).mockReturnValue(configWithExtras);
            await hookRegistry.initialize();
            // Should only load the valid hook
            expect(hookRegistry.getAllHooks()).toHaveLength(1);
            // Should skip all known config fields without warnings
            for (const field of HOOKS_CONFIG_FIELDS) {
                expect(mockDebugLogger.warn).not.toHaveBeenCalledWith(expect.stringContaining(`Invalid hook event name: ${field}`));
            }
            // Should warn on truly invalid event name
            expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith('warning', expect.stringContaining('Invalid hook event name: "InvalidEvent"'));
        });
    });
    describe('project hook warnings', () => {
        it('should check for untrusted project hooks when folder is trusted', async () => {
            ...
            vi.mocked(mockConfig.getHooks).mockReturnValue(projectHooks);
            vi.mocked(mockConfig.getProjectHooks).mockReturnValue(projectHooks);
            ...
            expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith('warning', expect.stringContaining('WARNING: The following project-level hooks have been detected'));
