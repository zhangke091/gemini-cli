...
describe('reportError', () => {
    ...
    it('should generate a report and log the path', async () => {
        ...
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(`${baseMessage} Full report available at: ${expectedReportPath}`, error);
    });
    ...
    it('should log fallback message if writing report fails', async () => {
        ...
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(`${baseMessage} Additionally, failed to write detailed error report:`, expect.any(Error));
    ...
    it('should handle stringification failure of report content (e.g. BigInt in context)', async () => {
        ...
        await reportError(error, baseMessage, context, type, testDir);
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(`${baseMessage} Could not stringify report content (likely due to context):`, stringifyError);
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith('Original error that triggered report generation:', error);
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith('Original context could not be stringified or included in report.');
        ...
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(`${baseMessage} Partial report (excluding context) available at: ${expectedMinimalReportPath}`, error);
    });
