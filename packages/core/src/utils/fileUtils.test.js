...
import { isWithinRoot, isBinaryFile, detectFileType, processSingleFileContent, detectBOM, readFileWithEncoding, fileExists, readWasmBinaryFromDisk, saveTruncatedToolOutput, formatTruncatedToolOutput, } from './fileUtils.js';
...
describe('fileUtils', () => {
    ...
    describe('detectFileType', () => {
        ...
        ])('should detect $type type for binary files with $ext extension', async ({ type, ext, mime }) => {
  ...
  <InterfaceList>
    ...
    describe('processSingleFileContent', () => {
        ...
        it('should process an audio file', async () => {
            ...
            const result = await processSingleFileContent(testAudioFilePath, tempRootDir, new StandardFileSystemService());
            expect(result.llmContent.inlineData).toBeDefined();
            expect(result.llmContent.inlineData
                .mimeType).toBe('audio/mpeg');
            expect(result.llmContent.inlineData.data).toBe(fakeMp3Data.toString('base64'));
  ...
  `;
    ...
    describe('saveTruncatedToolOutput & formatTruncatedToolOutput', () => {
        it('should save content to a file with safe name', async () => {
            ...
            const id = '123';
            const result = await saveTruncatedToolOutput(content, toolName, id, tempRootDir);
            const expectedOutputFile = path.join(tempRootDir, 'tool_output', 'shell_123.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
            expect(result.totalLines).toBe(1);
            const savedContent = await fsPromises.readFile(expectedOutputFile, 'utf-8');
        ...
        it('should sanitize tool name in filename', async () => {
            ...
            const expectedOutputFile = path.join(tempRootDir, 'tool_output', '______dangerous_tool_1.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
        });
        it('should sanitize id in filename', async () => {
            ...
            const expectedOutputFile = path.join(tempRootDir, 'tool_output', 'shell_______etc_passwd.txt');
            expect(result.outputFile).toBe(expectedOutputFile);
        });
        it('should format multi-line output correctly', () => {
            ...
            expect(formatted).toContain('Output too large. Showing the last 10 of 50 lines.');
        ...
        it('should handle massive single-line string with character-based truncation', () => {
            ...
            expect(formatted).toContain('Output too large. Showing the last 4,000 characters');
