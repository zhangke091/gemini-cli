...
export async function resolveExecutable(exe) {
        ...
        const tree = parser.parse(command, null, {
            progressCallback: () => {
                if (performance.now() > deadline) {
                    timedOut = true;
                    return true; // Returning true cancels parsing, but type says void
                }
            },
        });
        if (timedOut) {
 ...
 */
export function hasRedirection(command) {
 ...
 */
export async function* execStreaming(command, args, options) {
