...
import { escapeShellArg, getCommandRoots, getShellConfiguration, initializeShellParsers, parseCommandDetails, stripShellWrapper, hasRedirection, resolveExecutable, } from './shell-utils.js';
...
const mockPowerShellResult = (commands, hasRedirection) => {
    mockSpawnSync.mockReturnValue({
...
describe('hasRedirection (PowerShell via mock)', () => {
    ...
    it('should return false when quoted redirection chars are used but not actual redirection', () => {
        mockPowerShellResult([{ name: 'echo', text: 'echo "-> arrow"' }], false);
