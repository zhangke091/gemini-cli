...
import { classifyGoogleError, RetryableQuotaError, TerminalQuotaError, ValidationRequiredError, } from './googleQuotaErrors.js';
...
describe('classifyGoogleError', () => {
    ...
    it('should return RetryableQuotaError for long retry delays', () => {
        ...
        const result = classifyGoogleError(new Error());
        expect(result).toBeInstanceOf(RetryableQuotaError);
        expect(result.retryDelayMs).toBe(301000);
    });
    ...
    it('should classify nested JSON string 404 error as ModelNotFoundError', () => {
        ...
        const innerError = {
            error: {
                code: 404,
                message: 'models/NOT_FOUND is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.',
                status: 'NOT_FOUND',
            },
        };
        ...
        expect(classified.code).toBe(404);
    });
    ...
    it('should return ValidationRequiredError for 403 with VALIDATION_REQUIRED from cloudcode-pa domain', () => {
        ...
        expect(result.validationLink).toBe('https://example.com/validate');
        expect(result.validationDescription).toBe('Complete validation to continue');
        expect(result.learnMoreUrl).toBe('https://support.google.com/accounts?p=al_alert');
        expect(result.cause).toBe(apiError);
    });
    it('should correctly parse Learn more URL when first link description contains "Learn more" text', () => {
        ...
        const apiError = {
            ...
            details: [
                ...
                },
                {
                    ...
                    links: [
                        {
                            description: 'Further action is required to use this service. Navigate to the following URL to complete verification:\n\nhttps://accounts.sandbox.google.com/signin/continue?...\n\nLearn more:\n\nhttps://support.google.com/accounts?p=al_alert\n',
        ...
        expect(result.validationLink).toBe('https://accounts.sandbox.google.com/signin/continue?sarp=1&scc=1&continue=...');
    ...
    it('should fallback to ErrorInfo metadata when Help detail is not present', () => {
        ...
        const result = classifyGoogleError(new Error());
        expect(result).toBeInstanceOf(ValidationRequiredError);
        expect(result.validationLink).toBe('https://staging.example.com/validate');
        expect(result.validationDescription).toBeUndefined();
