 ...
 */
export function formatTruncatedToolOutput(contentStr, outputFile, truncateLines = 30) {
    ...
    if (totalPhysicalLines > 1) {
        ...
        const processedLines = lastLines.map((line) => {
            if (line.length > MAX_TRUNCATED_LINE_WIDTH) {
                ...
                return (line.substring(0, MAX_TRUNCATED_LINE_WIDTH) +
...
export async function saveTruncatedToolOutput(content, toolName, id, // Accept string (callId) or number (truncationId)
projectTempDir) {
